...
public:
  ...
  typedef slp::BucketVertexStorage VertexStorage;
  ...
  static EndomorphismSLP inverter(const TerminalSymbol& symbol) {
    ...
    return EndomorphismSLP(symbol, -symbol);
  }
  ...
  static EndomorphismSLP right_multiplier(const TerminalSymbol& symbol, const TerminalSymbol& right_multiplier) {
    ...
    return EndomorphismSLP(symbol, symbol, right_multiplier);
  }
  ...
  static EndomorphismSLP left_multiplier(const TerminalSymbol& left_multiplier, const TerminalSymbol& symbol) {
    ...
    return EndomorphismSLP(symbol, left_multiplier, symbol);
  }
  ...
  static EndomorphismSLP multiplication(const TerminalSymbol& multiplied, const TerminalSymbol& multiplier, bool is_left_multiplication) {
    ...
    if (is_left_multiplication) {
      return EndomorphismSLP(multiplied, multiplier, multiplied);
    } else {
      return EndomorphismSLP(multiplied, multiplied, multiplier);
    }
  }
  ...
  // Alias for *= operator
  EndomorphismSLP& compose_with(const EndomorphismSLP& a) {
    return (*this) *= a;
  }
  ...
  EndomorphismSLP conjugate_with(Iterator begin, Iterator end) const {
    ...
    return conjugator.compose_with(*this).compose_with(inverses.rbegin(), inverses.rend());
  }
  ...
  slp::VertexWord image_word(const TerminalSymbol& t) {
   ...
   */
  slp::Vertex image(const TerminalSymbol& t) {
    ...
    if (result == images_.end()) //if it is not in images_, then it is the identity map.
      return vertex_storage_.make_terminal(t);
    else
  ...
  bool is_identity() const {
    ...
    for_each_non_trivial_image([&is_id](const symbol_image_pair_type& pair) {
            if (is_id)
              is_id = (pair.first == TerminalVertex(slp::reduce(pair.second)).terminal_symbol());
          });
    return is_id;
  }
...
private:
   ...
   */
  slp::Vertex map_vertex(const slp::Vertex& vertex, const std::unordered_map<slp::Vertex, slp::Vertex>& images);

  //!Create simple Endomorphism mapping terminal from into terminal to
  EndomorphismSLP(TerminalSymbol from, TerminalSymbol to)
    : vertex_storage_()
    , images_{{
      from,
      vertex_storage_.make_terminal(to),
    }}
  { }

  //!Create simple Endomorphism mapping terminal from into the pair to_left, to_right
  EndomorphismSLP(TerminalSymbol from, TerminalSymbol to_left, TerminalSymbol to_right)
    : vertex_storage_()
    , images_{{
      from,
      slp::NonterminalVertex(
        vertex_storage_.make_terminal(to_left),
        vertex_storage_.make_terminal(to_right)
      )
    }}
  { }
