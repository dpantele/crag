...
TEST(HasherTest, SimpleCombinedHash) {
  ...
  auto a = VertexHash::make_terminal(1);
  auto b = VertexHash::make_terminal(2);

  EXPECT_NE(a, b);

  auto ab = VertexHash::make_concatenation(a, b);
  auto ba = VertexHash::make_concatenation(b, a);

  EXPECT_NE(ab, ba);

  auto b_1a_1 = VertexHash::make_concatenation(VertexHash::make_terminal(-2), VertexHash::make_terminal(-1));

  EXPECT_EQ(b_1a_1, VertexHash::make_inverse(ab));
}
